...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "nemesis"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("nemesis:"): self._tag = "ai"
...
_FILE_ICONS = {
    "image":   ("🖼", "#00ff41"), "video":   ("🎬", "#ff00ff"),
    "audio":   ("🎵", "#cc44ff"), "pdf":     ("📄", "#ff4444"),
    "word":    ("📝", "#4488ff"), "excel":   ("📊", "#44bb44"),
    "code":    ("💻", "#ffff00"), "archive": ("📦", "#ff8844"),
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for NEMESIS", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="NEMESIS", user_name="",
        ...
        self._hex_input.setPlaceholderText("#00ff41   (custom hex colour)")
    ...
    def _save(self):
        name = self._name_input.text().strip() or "NEMESIS"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Nemesis actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._qr_label.setStyleSheet(
            "color: #00ff41; background: #001a0d; border-radius: 10px;"
        )
        self._timer_lbl.setText("Phone connected — NEMESIS ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "NEMESIS").strip()
        ...
        self.setWindowTitle(f"[NEMESIS] — {_display}")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop NEMESIS mid-speech
        self.get_plugins       = None   # callable: () -> list[dict], set by NemesisLive
        self._muted            = False
        self._current_file: str | None = None
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_nemesis_icon(out_path: Path) -> bool:
        """
        Render a NEMESIS arc-reactor icon at 4× resolution and downsample
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "nemesis.ico"
        if not ico_path.exists():
            self._build_nemesis_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>NEMESIS</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.nemesis.assistant</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("Just A Rather Very Intelligent System"
                     if _disp in ("NEMESIS", "J.A.R.V.I.S")
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background: #050505; color: {C.WHITE};
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "NEMESIS_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.nemesis.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "nemesis.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "NEMESIS_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "NEMESIS_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.nemesis.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.nemesis.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "NEMESIS") or "NEMESIS",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "NEMESIS"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"[NEMESIS] — {_display}")
        self._title_lbl.setText(display)
        if display in ("NEMESIS", "J.A.R.V.I.S"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "NEMESIS") or "NEMESIS"
