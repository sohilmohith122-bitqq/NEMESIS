"""
dashboard/server.py — NEMESIS Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "NEMESIS Uploads",
        Path.home() / "Documents" / "NEMESIS Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'NEMESIS-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"NEMESIS Dashboard Port {port}"
        prog_rule  = "NEMESIS Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="nemesis_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until NEMESIS is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "nemesis.key"
    crt_p = certs / "nemesis.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "NEMESIS Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "NEMESIS"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "nemesis.key").exists() and (certs / "nemesis.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in NEMESIS to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('nemesis_token','{tok}');
  sessionStorage.setItem('nemesis_key','{key}');
  localStorage.setItem('nemesis_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to NEMESIS…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "nemesis.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "nemesis.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in NEMESIS UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
