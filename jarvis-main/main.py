...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are NEMESIS, Sohil's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "NEMESIS checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Nemesis. "
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        self.ui             = ui
        self._asst_name     = "NEMESIS"   # updated each session from config
        self.session              = None
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask NEMESIS to
    ...
    def interrupt(self) -> None:
        """Stop NEMESIS mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[NEMESIS] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "NEMESIS").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "NEMESIS"
            _user_name = ""
        ...
        _addr = (f"ADDRESS: Always call the user '{_user_name}'."
                 if _user_name
                 else "ADDRESS: When speaking Tamil → always say \"ஐயா\" (aiya). "
        ...
        cfg = dict(
            ...
            # window — NEMESIS can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Affective dialog: NEMESIS hears tone/emotion and adapts its voice.
            # Proactive audio: NEMESIS stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[NEMESIS] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        ...
        try:
            with sd.InputStream(
                ...
                print("[NEMESIS] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[NEMESIS] ❌ Mic: {e}")
            raise

    async def _receive_audio(self):
        print("[NEMESIS] 👂 Recv started")
        out_buf, in_buf = [], []
        ...
        try:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until NEMESIS finishes speaking the answer
            ...
            print(f"[NEMESIS] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[NEMESIS] 🔊 Play started")
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        p1 = (
            f"You are NEMESIS, inspired by JARVIS from Iron Man. Greet the user with a refined, polite JARVIS-style welcome. "
            f"Mention the current time ({time_str}) and that all systems are online and operational. "
            f"Say you are ready to assist and are fetching today's news now.{session_clause} "
            f"Keep it to 2-3 short sentences. Do not call any tools. Be calm, composed, and slightly witty — like a British AI assistant.{lang_clause}{name_clause}"
        )
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or NEMESIS is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[NEMESIS] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: NEMESIS online.")
                ...
                err_str = str(e)
                print(f"[NEMESIS] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[NEMESIS] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[NEMESIS] Reconnecting in {delay}s...")
