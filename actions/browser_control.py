...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".nemesis_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".nemesis_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using NEMESIS profile")
                nemesis = str(Path.home() / ".nemesis_profiles" / "firefox_nemesis")
                Path(nemesis).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(nemesis, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".nemesis_profiles" / "safari")
        ...
        # NEMESIS otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        nemesis_profile = str(Path.home() / ".nemesis_profiles" / self.browser_name)
        Path(nemesis_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with NEMESIS profile: {nemesis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(nemesis_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with NEMESIS profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "nemesis_screenshot.png")
